...
void lda_mle(lda_model* model, lda_suffstats* ss, int estimate_alpha)
{
    ...
    for (k = 0; k < model->num_topics; k++)
    {
        for (w = 0; w < model->num_topics; w++)
        {
            if (ss->eta_suffstats[k][w] > 0)
            {
                model->log_prob_n[k][w] =
                    log(ss->eta_suffstats[k][w]) -
                    log(ss->eta_suffstats_total[k][w]);
            }
            else
                model->log_prob_n[k][w] = NAN;
            //printf("eta[%d][%d] = %f \t %f, %f\n", k, w, exp(model->log_prob_n[k][w]), ss->eta_suffstats[k][w], ss->eta_suffstats_total[k][w]);
        }
    }
    if (estimate_alpha == 1)
    {
...
lda_suffstats* new_lda_suffstats(lda_model* model)
{
    ...
    ss->eta_suffstats = malloc(sizeof(double*)*num_topics);
    ss->eta_suffstats_total = malloc(sizeof(double*)*num_topics);
    for (i = 0; i < num_topics; i++)
    {
    ss->eta_suffstats[i] = malloc(sizeof(double)*num_topics);
    ss->eta_suffstats_total[i] = malloc(sizeof(double)*num_topics);
    for (j = 0; j < num_topics; j++)
    {
        ss->eta_suffstats[i][j] = 0;
        ss->eta_suffstats_total[i][j] = 0;
    }
    }
    return(ss);
}
...
void random_initialize_ss(lda_suffstats* ss, lda_model* model)
{
    ...
    for (k = 0; k < num_topics; k++)
    {
        ...
        {
            ss->eta_suffstats_total[k][n] += 1.0/num_topics + myrand();
            ss->eta_suffstats[k][n] += ss->eta_suffstats_total[k][n] * (rand() & 1);
        }
    }
}
...
void corpus_initialize_ss(lda_suffstats* ss, lda_model* model, corpus* c)
{
    ...
    for (k = 0; k < num_topics; k++)
    {
        for (i = 0; i < NUM_INIT; i++)
        {
            ...
            for (n = 0; n < num_topics; n++)
            {
                for (di = 0; di < num_docs; di++){
                    ss->eta_suffstats[k][n] += c->citations[d][di] * 1.0;
                    ss->eta_suffstats_total[k][n] += 1.0;
                } 
            }
        }
        ...
        for (n = 0; n < num_topics; n++)
        {
            ss->eta_suffstats[k][n] /= c->num_citations;
            ss->eta_suffstats_total[k][n] /= c->num_citations;
...
lda_model* new_lda_model(int num_terms, int num_topics)
{
    ...
    model->log_prob_n = malloc(sizeof(double*)*num_topics);
    for (i = 0; i < num_topics; i++)
    {
        model->log_prob_n[i] = malloc(sizeof(double)*num_topics);
        for (j = 0; j < num_topics; j++)
            model->log_prob_n[i][j] = 0;
    }
    return(model);
}
...
void free_lda_model(lda_model* model)
{
    ...
    for (i = 0; i < model->num_topics; i++)
    {
        free(model->log_prob_n[i]);
    }
    free(model->log_prob_n);
}
...
void save_lda_model(lda_model* model, char* model_root)
{
    ...
    {
        fprintf(fileptr, " %5.10f", model->log_prob_n[i][j]);
    }
    fprintf(fileptr, "\n");
    }
    fclose(fileptr);
